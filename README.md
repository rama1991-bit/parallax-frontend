...
- Phase 2 ingested-article analysis, article-id compare with title, missing/added claim, coverage-gap, source, and timeline detail, event-cluster-aware compare context with cluster quality, language-bridge terms, coverage tasks, suggested source searches, source draft links/status, source draft discovery/create/ignore/resolve actions, source candidate validation, one-click candidate onboarding, batch candidate onboarding, retryable onboarding runs, review-gated ingestion controls, source workflow history/timelines, source candidate prefill, immediate sync/intelligence/cluster refresh after add, node-based article detail, bounded OSINT panels, default source seeding, source/feed creation forms, per-source sync controls, pending article analysis controls, active RSS/homepage source sync controls with manual-feed skip summaries, source/topic intelligence aggregation panels, topic detail event cluster panels, intelligence refresh/run controls, cluster refresh/run controls, one-click admin pipeline run controls, intelligence feed cards, ingestion health badges, source quality badges, source operational alerts, alert delivery status/actions, provider metadata badges, and admin source/feed governance controls
